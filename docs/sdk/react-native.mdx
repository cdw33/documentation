...
Passing a value of `false` into `requestPermissions` will only prompt for foreground location permissions.
...
- **`CONTINUOUS`**: A high frequency of location updates and higher battery usage. Suitable for on-demand use cases (e.g., delivery tracking) and some consumer use cases (e.g., order ahead, "mall mode"). In continous mode, Android devices start a foreground service, and iOS shows a blue bar.
